 ...
 */
function showData(DATA_URL) {
  // El for itera sobre los elementos del array
  for (const student of DATA_URL) {
    ...
    container.innerHTML += `<p> ${student.name} ${student.lastname} </p>`; // Se concatena cada párrafo de la manera que queremos mostrarlo al innerHTML del contenedor
  }
}
...
.then(data => {
  showData(data.students); // Llama a showData con la lista de estudiantes
  })
  .catch(error => {
    console.error('Hubo un problema al hacer fetch', error);
  });
